...
class RecyclerViewDataAdapter(private val mContext: Context, private val dataList: ArrayList<SectionDataModel>?) : RecyclerView.Adapter<RecyclerViewDataAdapter.ItemRowHolder>() {
    ...
    override fun onBindViewHolder(itemRowHolder: ItemRowHolder, i: Int) {
        ...
        val sectionName = dataList!![i].getHeaderTitle()

        val singleSectionItems = dataList[i].getAllItemsInSection()
        ...
        itemRowHolder.btnMore.setOnClickListener { v -> Toast.makeText(v.context, "click event on more, $sectionName", Toast.LENGTH_SHORT).show() }
